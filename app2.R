...
ui <- fluidPage(
  titlePanel("SpectraCSV"),
  
  sidebarLayout(
    sidebarPanel(
      tabsetPanel(
        tabPanel("Examinar CSV",  # Cambiado a la primera pestaña
                 fileInput("exam_csv", "Seleccionar archivo CSV para examinar", accept = c(".csv")),
                 tableOutput("examined_data")
        ),
        tabPanel("Unir CSV",  # Cambiado a la segunda pestaña
        ...
        tabPanel("Visualizador de Firmas Espectrales",  # Cambiado a la tercera pestaña
