...
ruta <- "C:/Users/Usuario/Documents/Experimentos_2024/app_signature_analysis/spec_analysis/datos/texto_sin_chapear/no_chapeado_ischaemum.dat.csv"
...
spectro_analysis_resume <- function(df) {
  ...
  # Crear el gráfico
  plot <- ggplot(summary_results, aes(x = nm, y = mean_reflec, color = id, group = interaction(id, banda))) +
    geom_line() +  # Añadir las líneas por id
    geom_errorbar(aes(ymin = mean_reflec - se_reflec, ymax = mean_reflec + se_reflec), width = 0.2) +  # Añadir barras de error
    labs(title = "Espectro de Reflectancia por ID con Error Estándar",
         x = "Longitud de Onda (nm)",
         y = "Reflectancia Media") +
    theme_minimal() +
    
    # Añadir líneas verticales para delimitar cada banda espectral
    ...
    # Añadir etiquetas de texto para las bandas
