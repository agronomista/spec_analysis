...
ui <- fluidPage(
  ...
  sidebarLayout(
    sidebarPanel(
      ...
      helpText("Por favor, ingrese el prefijo original que desea reemplazar en los ID's:"),
      textInput("prefix_old_rsc", "Prefijo original para 'IS-RSC':", value = "IS-RSC"),
      
      helpText("Ingrese el nuevo prefijo con el que desea reemplazar el original:"),
      textInput("prefix_new_rsc", "Nuevo prefijo para 'IS-RSC':", value = "IS_RESISTENTE"),
      
      helpText("Por favor, ingrese el prefijo original que desea reemplazar en los ID's secundarios:"),
      textInput("prefix_old_ssc", "Prefijo original para 'IS-SSC':", value = "IS-SSC"),
      
      helpText("Ingrese el nuevo prefijo con el que desea reemplazar el original en los ID's secundarios:"),
      textInput("prefix_new_ssc", "Nuevo prefijo para 'IS-SSC':", value = "IS_SUSCEPTIBLE"),
...
server <- function(input, output) {
  ...
  observeEvent(input$process, {
    ...
    # Cambiar de comas a puntos y convertir a numérico
    test_open[, 4] <- gsub(",", ".", test_open[, 4])
    test_open[, 4] <- as.numeric(test_open[, 4])
    
    # Cambiar el prefijo de los ID´s usando los valores proporcionados por el usuario
    test_open[, 1] <- gsub(paste0("^", input$prefix_old_rsc, ".*"), input$prefix_new_rsc, test_open[, 1])
    test_open[, 1] <- gsub(paste0("^", input$prefix_old_ssc, ".*"), input$prefix_new_ssc, test_open[, 1])
...
plot_band_by_range <- function(summary_results, banda_name, nm_start, nm_end) {
  p <- ggplot(subset(summary_results, nm >= nm_start & nm <= nm_end), 
    ...
    geom_line() +  
    geom_errorbar(aes(ymin = mean_reflec - se_reflec, ymax = se_reflec + mean_reflec), width = 0.2) +  
...
spectro_analysis_resume <- function(df) {
  summary_results <- df %>%
    ...
    summarise(
      ...
      n = n(),
      se_reflec = sd(reflec, na.rm = TRUE) / sqrt(n()),
